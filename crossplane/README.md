...
| `xrd/definition.yaml` | The `XStorageAccount` XRD - the schema a claim must match: `name`, `location`, `sku`. `resourceGroupName` is deliberately not a field here. |
| `xrd/composition.yaml` | Pipeline-mode `Composition` (via `function-patch-and-transform`) that turns a claim into a real `storage.azure.upbound.io` `Account`, landing it in the one pre-scoped resource group from Terraform - hardcoded in this file, not passed through from the claim. |
